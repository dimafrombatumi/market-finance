-- Complete Workshop Management System Schema for Supabase
-- This file contains the complete database schema for workshop management and registrations
...
CREATE INDEX IF NOT EXISTS idx_workshop_analytics_workshop_id ON workshop_analytics(workshop_id);
CREATE INDEX IF NOT EXISTS idx_workshop_analytics_date ON workshop_analytics(date);
...
-- Function to update workshop statistics
CREATE OR REPLACE FUNCTION update_workshop_stats()
RETURNS TRIGGER AS $$
BEGIN
  IF TG_OP = 'INSERT' THEN
  ...
  ELSIF TG_OP = 'DELETE' THEN
    ...
    RETURN OLD;
  ELSIF TG_OP = 'UPDATE' THEN
    IF OLD.workshop_id != NEW.workshop_id THEN
      -- Decrease count for old workshop
      UPDATE workshops 
      SET total_registrations = total_registrations - 1
      WHERE id = OLD.workshop_id;
      -- Increase count for new workshop
      UPDATE workshops 
      SET total_registrations = total_registrations + 1
      WHERE id = NEW.workshop_id;
    END IF;
...
-- Trigger to update workshop statistics
CREATE TRIGGER update_workshop_stats_trigger
  AFTER INSERT OR UPDATE OR DELETE ON workshop_registrations
  FOR EACH ROW EXECUTE FUNCTION update_workshop_stats();

-- Function to update workshop and instructor ratings
CREATE OR REPLACE FUNCTION update_workshop_rating()
RETURNS TRIGGER AS $$
DECLARE
  workshop_rating DECIMAL(3,2);
  instructor_rating DECIMAL(3,2);
  workshop_reviews_count INTEGER;
  instructor_reviews_count INTEGER;
  target_workshop_id UUID;
  target_instructor_id UUID;
BEGIN
  -- Get workshop and instructor IDs
  IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
    target_workshop_id := NEW.workshop_id;
  ELSE
    target_workshop_id := OLD.workshop_id;
  END IF;
  
  -- Get instructor ID for the workshop
  SELECT instructor_id INTO target_instructor_id
  FROM workshops 
  WHERE id = target_workshop_id;
  
  -- Update workshop rating
  SELECT AVG(rating)::DECIMAL(3,2), COUNT(*)
  INTO workshop_rating, workshop_reviews_count
  FROM workshop_reviews 
  WHERE workshop_id = target_workshop_id AND is_public = true;
  
  UPDATE workshops 
  SET rating = COALESCE(workshop_rating, 0), 
      total_reviews = workshop_reviews_count
  WHERE id = target_workshop_id;
  
  -- Update instructor rating (average of all their workshops' ratings)
  IF target_instructor_id IS NOT NULL THEN
    SELECT AVG(w.rating)::DECIMAL(3,2), COUNT(DISTINCT r.id)
    INTO instructor_rating, instructor_reviews_count
    FROM workshops w
    LEFT JOIN workshop_reviews r ON w.id = r.workshop_id AND r.is_public = true
    WHERE w.instructor_id = target_instructor_id;
    
    UPDATE instructors 
    SET rating = COALESCE(instructor_rating, 0), 
        total_reviews = instructor_reviews_count
...
CREATE TRIGGER update_workshop_rating_trigger
  AFTER INSERT OR UPDATE OR DELETE ON workshop_reviews
...
-- RLS policies for workshop_analytics
CREATE POLICY "Analytics are viewable by authenticated users" ON workshop_analytics
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "Analytics are manageable by authenticated users" ON workshop_analytics
  FOR ALL USING (auth.role() = 'authenticated');
...
-- Create function to get workshop statistics
CREATE OR REPLACE FUNCTION get_workshop_stats(workshop_uuid UUID)
RETURNS JSON AS $$
DECLARE
  result JSON;
  reg_stats RECORD;
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(SUM(wr.total_amount), 0) as total_revenue
  INTO reg_stats
  FROM workshop_registrations wr
  WHERE wr.workshop_id = workshop_uuid;
  ...
  INTO review_stats
  FROM workshop_reviews
  WHERE workshop_id = workshop_uuid AND is_public = true;
  
  -- Build result
  result := json_build_object(
    'total_registrations', reg_stats.total_registrations,
    'confirmed_registrations', reg_stats.confirmed_registrations,
    'completed_registrations', reg_stats.completed_registrations,
    'total_revenue', reg_stats.total_revenue,
...
-- Create function to check workshop availability
CREATE OR REPLACE FUNCTION check_workshop_availability(schedule_uuid UUID)
RETURNS BOOLEAN AS $$
DECLARE
...
BEGIN
  SELECT ws.max_participants, ws.current_participants
  INTO max_participants, current_participants
  FROM workshop_schedules ws
  WHERE ws.id = schedule_uuid;
